...
//        collectionView.registerReusableView(viewClass: MyCollectionSection.self, forSupplementaryViewOfKind:UICollectionElementKindSectionFooter)
        ...
        collectionView.ac.adaptor?.appendToLast(section:sectionHolder, datas: ["row:1","row:2","row:3",], cellClass: MyCollectionCell.self, cellHolderClass: MyCollectionCellViewHolder.self)
        collectionView.ac.adaptor?.dataSource.append(contentsOf: getData())
        collectionView.ac.adaptor?.appendToLast(datas: ["row:5","row:5","row:6",], cellClass: MyCollectionCell.self, cellHolderClass: MyCollectionCellViewHolder.self)
        testSectionSubscrib()
        testAdaptorSubscrib()
...
//        layout.footerReferenceSize = CGSize(width: 44, height: 44)
    ...
    func testSectionSubscrib(){
        collectionView.ac.adaptor?.dataSource[0][0].cellData = "test1"
        collectionView.ac.adaptor?.dataSource[0][1] = "test2"
    }
    
    func testAdaptorSubscrib() {
        collectionView.ac.adaptor?[IndexPath(item: 0, section: 1)].cellData = "AdTest1"
        collectionView.ac.adaptor?[IndexPath(item: 1, section: 1)].cellData = "AdTest2"
    }
}
